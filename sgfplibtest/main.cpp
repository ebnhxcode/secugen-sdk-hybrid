...
int main(int argc, char **argv) 
{
  ...
  if (err == SGFDX_ERROR_NONE)
  {
    ...
    if (err == SGFDX_ERROR_NONE)
    {
      ...
      strcpy(function,"GetNumOfMinutiae()");
      printf("\nCall %s\n",function);
      err = sgfplib->GetNumOfMinutiae(TEMPLATE_FORMAT_ANSI378, minutiaeBuffer1, &numOfMinutiae);
      printf("%s returned: %ld\n",function,err);
      printf("Minutiae Count : [%ld]\n",numOfMinutiae);
    ...
    if (err == SGFDX_ERROR_NONE)
    {
      ...
      strcpy(function,"GetNumOfMinutiae()");
      printf("\nCall %s\n",function);
      err = sgfplib->GetNumOfMinutiae(TEMPLATE_FORMAT_ISO19794, minutiaeBuffer1, &numOfMinutiae);
      printf("%s returned: %ld\n",function,err);
      printf("Minutiae Count : [%ld]\n",numOfMinutiae);
